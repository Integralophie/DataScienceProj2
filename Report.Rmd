...
The data on fatalities are collected from air crash investigation reports, and may be able to assume that there are no sampling uncertainties in the collection of data, because our dataset is pretty close to the population. However, we also believe that there's a underestimate of the number of air crashes: as we look through the dataset, it seems that not all of the crashes are represented in the dataset. For example, during the terrorist attack on September 11, 2001, four airplanes are hijacked and crashed, and the dataset only records one of them. We do not know how many other accidents went unreported.
