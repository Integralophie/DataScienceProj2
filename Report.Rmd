...
The data on fatalities are collected from air crash investigation reports, and we can confidently assume that there are no sampling uncertainties in the collection of data, because our dataset is the population. However, we also believe that there's a underestimate of the number of air crashes: as we look through the dataset, it seems that not all of the crashes are represented in the dataset. For example, during the terrorist attack on September 11, 2001, four airplanes are hijecked and crashed, and the dataset only records one of them. 
...
We noticed from the previous figure that in June, July and August, there is a peak in air crashes. We thus made a hypothesis that the more frequent severe weather including thunderstroms are microbursts during that time period contributes to the higher frequency of air crashes. 
